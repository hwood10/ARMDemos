...
Param(
  ...
  [string] $ResourceGroupName = 'EMJU-AZDV-OfferSetup-VMs',  
  [switch] $UploadArtifacts,
  [string] $StorageAccountName,
  [string] $StorageAccountResourceGroupName, 
  [string] $StorageContainerName = $ResourceGroupName.ToLowerInvariant() + '-stageartifacts',
  [string] $TemplateFile = '..\Templates\DeploymentTemplate-EMJU.PublicService-VMs.json',
  [string] $TemplateParametersFile = '..\Templates\DeploymentTemplate-EMJU.PublicService-VMs-OfferSetup.param.dev.json',
