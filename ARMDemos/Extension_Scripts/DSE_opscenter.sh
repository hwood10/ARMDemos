...
#
# 18 IP addresses are researved per data center, where the first starts with 10.0.200.10 
# Reseaving 10.0.200.4-6 for OpsCenter and other nodes
#
...
if [ $testing == 'true' ] ; then 
  echo API $NODE_INFORMATION_API
  echo ANALYTICS $NODE_INFORMATION_ANALYTICS 
fi
...
#
# Use this pattern to add a datacenter
#
# curl --insecure -H "Accept: application/json" -X POST http://127.0.0.1:8888/{cluster_id}/provision -d @provision.json
